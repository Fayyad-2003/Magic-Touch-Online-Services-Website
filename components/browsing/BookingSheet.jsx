...
const Booking = ({ children, serviceData }) => {
    ...
    const endMonth = `${date.getFullYear()}-${date.getMonth() + 3}`;
    ...
    return (
        <Sheet>
            ...
            <SheetContent className='overflow-y-scroll bg-background-color border-none'>
                <SheetHeader>
                    <SheetTitle className='border-b-2 border-primary pb-3 text-primary text-start'>
                        Book a Service
                    </SheetTitle>

                    <SheetDescription>
                        {/* Instructions and notes */}
                        <p className='text-primary/70 mt-2 text-[12px] text-start tracking-[1.2px]'>
                        ...
                        {/* Time slot selection */}
                        <h2 className='text-primary mt-5 text-lg text-start'>Select Time Slot</h2>
                        ...
                        <div className='grid grid-cols-2 md:grid-cols-3 gap-3 mt-6'>
                            ...
                            {timeSlot.map((time, index) => (
                                <Button
                                    ...
                                    className={`bg-background-color flex flex-col -gap-[2px] text-primary border-primary hover:text-white cursor-pointer ${
                ...
                <SheetFooter className='mt-5'>
                    <SheetClose asChild>
                        <div>
                            ...
                            {/* Cancel button */}
                            <Button variant='outline' className='ml-3 bg-background-secondary-color/60 hover:bg-background-secondary-color border-none text-light-blue-color cursor-pointer'>
                                Cancel
                            </Button>
                        </div>
                    </SheetClose>
