...
describe('Booking Component', () => { // Changed from BookingSheet to Booking
  ...
  // Mock serviceBookedSlot function
const mockServiceBookedSlot = jest.fn();
  ...
  test('allows selecting a date and time slot', async () => {
    ...
    // Select a time slot
    const timeSlotButton = screen.getByRole('button', { name: '10:00 AM' });
    fireEvent.click(timeSlotButton);

    // Assert that the time slot is selected (checking class name)
    expect(timeSlotButton).toHaveClass('bg-primary');
  });
  ...
  test('disables already booked time slots', async () => {
    ...
    mockServiceBookedSlot.mockResolvedValue({
      bookings: [{ time: '10:30 AM' }], // Mock one booked slot
    });

    render(
      <BookingSheet serviceData={mockServiceData} serviceBookedSlot={mockServiceBookedSlot}>
        <button>Open Booking</button>
      </BookingSheet>
    );
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: '10:30 AM' })).toBeInTheDocument());

    // Check if the booked slot is disabled
    const bookedSlotButton = screen.getByRole('button', { name: '10:30 AM' });
    expect(bookedSlotButton).toBeDisabled();
  });
  ...
  test('prevents booking for past dates', () => {
    ...
    const yesterdayGridCell = screen.getAllByRole('gridcell', { name: '1' })[0];
    const yesterdayButton = yesterdayGridCell.querySelector('button');
  ...
  test('disables past time slots when the selected date is today', () => {
    ...
    const pastTimeSlot = screen.getByRole('button', { name: '10:00 AM' });
    expect(pastTimeSlot).toBeDisabled();

    const futureTimeSlot = screen.getByRole('button', { name: '6:00 PM' });
  ...
  test('enables all time slots when the selected date is not today', async () => {
    ...
    mockServiceBookedSlot.mockResolvedValue({ bookings: [] });
    ...
    // Get all time slot buttons using getAllByRole with options
    const timeSlotButtons = screen.getAllByText(/^(1[0-2]|[1-6]):[0-5][0-9] (AM|PM)$/,);
